...
export default function Servicios() {
    ...
    const { mutate: deleteService } = useMutation({
        ...
        onError: () => {
            toast.error('No se puede eliminar porque tiene visitas asociadas');
        }
    });
    ...
    const handleDelete = (id: string, name: string) => {
        if (window.confirm(`¿Seguro que querés eliminar el servicio "${name}"?`)) {
            deleteService(id);
        }
    };
                    ...
                    {servicios?.map((servicio) => (
                        <div key={servicio._id} className="bg-maison-card border border-maison-border rounded-2xl p-6 shadow-sm hover:shadow-md transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                <div className="bg-maison-bg p-3 rounded-xl border border-maison-border text-gray-600">
                                    <FiScissors className="text-xl" />
                                </div>
                                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleEdit(servicio)} className="p-2 text-gray-400 hover:text-maison-primary transition-colors cursor-pointer">
                                        <FiEdit2 size={16} />
                                    </button>
                                    <button onClick={() => handleDelete(servicio._id, servicio.name)} className="p-2 text-gray-400 hover:text-maison-red transition-colors cursor-pointer">
                                        <FiTrash2 size={16} />
                                    </button>
                                </div>
                            </div>

                            <h4 className="text-xl font-serif text-maison-text mb-3">
                                {servicio.name}
                            </h4>
                            ...
                            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs font-semibold uppercase tracking-widest text-gray-500">
                                <FiClock />
                                {servicio.defaultTouchupDays > 0
                                    ? `Retoque en ${servicio.defaultTouchupDays} días`
                                    : 'Sin retoque'
                                }
                            </div>
                        </div>
